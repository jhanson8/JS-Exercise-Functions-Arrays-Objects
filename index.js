...
function makeSmartPerson(name) {
  const objectOne = {
    ...
    speak: function() {
      return "Hello, my name is " + objectOne.name;
    }
  };
  return objectOne;
}
...
function sortCarInventory(inventory) {
  inventory.sort(function(a, b) {
    var modelOne = a.car_model;
    var modelTwo = b.car_model;
    return modelOne < modelTwo ? -1 : modelOne > modelTwo ? 1 : 0;
  });
...
function getGermanCars(inven) {
  ...
  for (let i = 0; i < inven.length; i++) {
    testObj = inven[i];
    for (let x = 0; x < german.length; x++) {
      if (testObj.car_make == german[x]) {
        newInven.push(testObj);
      }
    }
  }
  return newInven;
